 ...
 * Only what is raised changes face. It is there for the punctuation that belongs
...
class SuperscriptInlayRenderer(
    ...
    /**
     * Sans, shrunk, and never the editor's face.
     *
     * A raised marker is not prose but a label on it, and the scheme sets the
     * prose in a serif: leaving the marker in the same face makes it read as a
     * word that was typed, only smaller. [RenderFonts] is where the sticky notes
     * and the attribute strips already get their face, so the renderings that
     * are not source all speak with one voice -- and the family stays the
     * author's to change, defaulting to sans.
     *
     * The size still follows the editor's, so a marker grows and shrinks with
     * Ctrl+Wheel along with the text it sits above.
     */
    private fun markerFont(editor: Editor): Font = RenderFonts.font(editor, SCALE)
